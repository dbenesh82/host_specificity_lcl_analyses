...
**Background**: I published a [database](http://onlinelibrary.wiley.com/doi/10.1002/ecy.1680/suppinfo) of helminth (parasitic worm) life cycles. Complex life cycle parasites infect dissimilar hosts over the course of their life cycles. This script is to get the taxonomic data for the hosts to calculate host specificity indexes.
...
To get the taxonomies, we have to query the NCBI for each host species. This code is simply to get an idea how long it takes. It's blocked out because it does not need to be run everytime.
...
This returns a list for each species that includes a 'uid', the unique identifier in NCBI. We want to extract that, as well as some other data from the list. The next block loops through each species to extract ids, string queried (species or genus name), and taxonomic level of the query (species or genus). When host species were not found in the NCBI database, the genus name was queried. 
...
Let's look at missing data in the classical taxonomic hierarchy (genus, family, order, class, phylum). Quite a few host species lack information for order or class.

```{r}
sapply(select(ncbi.tax.out, genus, family, order, class, phylum), function(x) sum(is.na(x)))
```

Instead of ignoring species with this missing taxonomic information, we can try to impute it with a quick and dirty algorithm. We'll create a couple functions to help execute this. The first function returns other taxonomic ranks (e.g. subgenus) that straddle the common ones (genus)
...
The function `get_straddling_tax_levels` is a helper in next function, which replaces missing taxonomic ranks with one of the straddling ranks. Sometime multiple straddling ranks will be available, but it returns the most common one.
...
```{r}
# create a df which records the number of times any given taxonomic name appears
# this is used to judge which taxa is the best for filling in missing data
...
Then we fill in the missing data.
