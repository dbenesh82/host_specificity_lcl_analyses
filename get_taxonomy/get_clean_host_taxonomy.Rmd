...
For 3500+ host species, we query the NCBI database to get the uniques ids for each. It takes a while. The last time this script was run and the database was queried was `r Sys.time()`.
...
Check how much was filled in. Quite an improvement. Thus, for species where some taxonomy was returned, then it was possible to 
